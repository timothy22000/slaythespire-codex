...
Slay the Spire 1 + 2 card dataset and embedding pipeline. Produces ML/DL training-ready HuggingFace datasets (one cards repo and one embeddings repo per game) with normalized card metadata, derived feature columns, and 2026-current text embeddings — so downstream users can plug straight into a classifier, retriever, or model fine-tune without further preprocessing.
